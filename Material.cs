    ...
    public partial class Material : Form
    {
        ...
        private void InitData()
        {
            try
            {
                ...
                con.Open();
                cmd = new NpgsqlCommand("SELECT * FROM market", con);
        ...
        {
            dataGridView1.SelectionChanged -= new EventHandler(dataGridView1_SelectionChanged);
            strSQL = string.Format("INSERT INTO Материал(Название,\"Цена за 1м\") VALUES('{0}', {1})",
        ...
        {
            dataGridView1.SelectionChanged -= new EventHandler(dataGridView1_SelectionChanged);
            strSQL = string.Format("UPDATE Материал SET " +
                "Название='{0}', \"Цена за 1м\"={1} " +
                "where \"ID материала\"={2}", textBox2.Text, textBox3.Text, textBox1.Text);
            try
            {
                con = new NpgsqlConnection(connectionString);
                con.Open();
        ...
        {
            dataGridView1.SelectionChanged -= new EventHandler(dataGridView1_SelectionChanged);
            strSQL = string.Format("DELETE FROM Материал WHERE \"ID материала\"={0}", textBox1.Text);
            try
            {
                con = new NpgsqlConnection(connectionString);
                con.Open();
        ...
        {
            if (textBox1.Text != "")
                strSQL = string.Format("select * from Материал where \"ID материала\"={0}", textBox1.Text);
            else if (textBox2.Text != "")
                strSQL = string.Format("select * from Материал where Название='{0}'", textBox2.Text);
            else if (textBox3.Text != "")
                strSQL = string.Format("select * from Материал where \"Цена за 1м\"={0}", textBox3.Text);
            if (strSQL != "")
            {
                dataGridView1.SelectionChanged -= new EventHandler(dataGridView1_SelectionChanged);
                try
